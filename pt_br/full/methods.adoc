...
solution to Conway's Game of Life from <<arrays#arrays>>. If we
...
statements or loops. Calling methods inside of methods is also allowed.
In the `max()` method, we use an `if`-`else` construction to find
...
of a method, we could have written the method without an `else`.
...
<<classes#classes>>. Unlike the static methods in this chapter,
...
problem. In <<synchronization#synchronization>> we'll talk about the much
