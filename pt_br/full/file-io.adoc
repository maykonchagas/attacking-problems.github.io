[#file-io]
:imagesdir: chapters/file-io/images
:programsdir: chapters/file-io/programs
== File I/O
...
order to reach some files, you probably had to look through directories (often
called folders), which are a special kind of file as well. But what is a
file?
...
secondary storage such as flash drives, hard drives, and optical media
...
Likewise, _all_ input and output on Unix and Linux systems is viewed as
...
is, when you open a text file with a simple text editor, it won't be
...
In some sense, the idea of a text file is artificial. _All_ files are
...
the Java API https://docs.oracle.com/en/java/javase/21/docs/api/index.html[documentation site^],
you can get detailed documentation for the entire standard library, including
https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/nio/file/package-summary.html[file I/O classes^].
...
If we want to write a list of 100 random numbers to the file we were reading
from earlier, we could do it as follows.
...
Programmers new to file I/O are sometimes unsure what will happen when an
existing file is opened for writing. Will new content be written at the end of
the old file? Will it overwrite the data, line by line?

While there are Java tools that will allow output to be appended to the end of a
file, the default for most output, including `PrintWriter`, is to destroy
everything inside the file when opening it for writing. Thus, if you're
wondering where all your old data went after writing some more data to the file,
it's gone.

Be especially careful when opening data files that can't easily be recreated
since there might not be any way to retrieve the data. Remember: Opening a file
for reading is safe, but opening a file for writing will usually delete all its
existing contents.
====
...
We discussed fully featured GUI programs in <<gui#gui>>, but like the
`JOptionPane` class covered in <<gui-basics#gui-basics>>,
...
<.> This constant tells us that the user selected a file rather than canceling.
<.> We get a `File` object from the file chooser and then convert it into a 
`Path`.
<.> If the file exists, we get its size in bytes and print that out.
...
The purpose of `JFileChooser` is to allow users to select a file.  It
...
----
<.> The code first creates a `Path` object using `"."` to
specify the current working directory.
<.> The `Files.list()` method returns a stream of `File` objects which we can
process. Refer to <<Advanced: Streams>> for more information about streams.
<.> We use two more method calls in `Files` to get the time each file was
...
<.> We then use the `isDirectory()` method to see if the file is a directory.
<.> Finally, we print the name of  the file without any preceding path, given by
 `getFileName()`.
...
----
<.> Inside the `try`-with-resources, we create a `Scanner` to read text from a
file.
<.> We also create a `PrintWriter` to write text to a file. Because both file
...
keyboard.
<.> We use the `format()` method to output to a file a `double` formatted with
exactly three digits after the decimal point followed by a newline, just as
we've used `System.out.format()` in the past.
<.> As is typical with file I/O, we have to catch exceptions. An 
...
`DataInputStream`.
<.> We change the `PrintWriter` to a `DataOutputStream`. Again, both objects are
...
[source, java]
----
include::{programsdir}/BitmapCompression.java[lines=1..18]
----
<.> We first check that there are exactly two command-line arguments.
Otherwise, our program would crash if we try to access invalid array
...
<.> The `catch` will print an error message if a file can't be opened, read, or
written.

[source, java]
----
include::{programsdir}/BitmapCompression.java[lines=20..46]
----
...
to finish.
<.> The method finishes with the usual `catch` for error cases.

[source, java]
----
include::{programsdir}/BitmapCompression.java[lines=48..64]
----
...
<.> An additional `catch` deals with errors.
...
operations are operating system dependent. Although Java tries to give a uniform
...
https://www.gutenberg.org[Project Gutenberg^] and a dictionary file from an
