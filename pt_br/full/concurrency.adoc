[#concurrency]
:imagesdir: chapters/concurrency/images
:programsdir: chapters/concurrency/programs
...
o trabalho a ser feito é o cerne de qualquer solução concorrente para um problema.
...
problema será abordada no <<synchronization#synchronization>>. Quando pudermos 
...
==== Arquiteturas de memória e concorrência
...
[source, java]
----
include::{programsdir}/SumThread.java[lines=17..23]
----
...
[source, java]
----
include::{programsdir}/SumThread.java[lines=25..43]
----
...
[source, java]
----
include::{programsdir}/SumThread.java[lines=45..56]
----
...
|=======================================================================
|Size (_n_) |_t_~_s_~ (s) |_t_~_c_~ (s) |Speedup
|100 |0.013 |0.9 |0.014
...
=== Conceitos: Agendamento de thread
...
programador. Quando as threads são agendadas e em qual processador eles são executados, isso 
...
métodos `Thread.sleep()` e `Thread.yield()`, que discutiremos em <<Sintaxe: Estados da thread>> e 
...
.  [[speedupExercise]] Considere a definição de velocidade de <<Exemplos: Concorrência e aceleração>>. Vamos supor que você tenha um
