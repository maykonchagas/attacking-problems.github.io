...
:imagesdir: chapters/testing-and-debugging/images
:programsdir: chapters/testing-and-debugging/programs
== Testing and Debugging
...
will be discussed at length in <<polymorphism#polymorphism>>. The
...
<<synchronization#synchronization>>. Beyond deadlocks and
...
JUnit is used for _unit testing_ in Java. Unit testing is the process
...
has simple syntax for creating JUnit tests that relies on annotations, first
mentioned in <<Annotations>>. Annotations are additional information
written into Java code that affects how the compiler or run-time system treats
the code. In JUnit, annotations are used to mark the code that makes up tests,
test suites, and other activities that should be done before or after running
tests.
...
To use a method for a JUnit test, it must be marked with `@Test`. It's also
...
So far we've talked about every aspect of writing a JUnit test
except for the contents of the test itself. How does the JUnit test report a
