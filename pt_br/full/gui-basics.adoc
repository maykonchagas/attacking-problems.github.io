...
Input:::
  ...
  arbitrary text. See <<inputDialogTextFigure>> for an
  example.
...
<<Sintaxe: Noções básicas de Java>>, but we give two more examples here to
...
given in <<Solução: Calculadora de custos universitários>> so that it uses
GUIs constructed with `JOptionPane` for input and output.
. Re-implement the solution to the Monty Hall problem given in
<<Solução: Monty Hall>> so that it uses GUIs constructed with
