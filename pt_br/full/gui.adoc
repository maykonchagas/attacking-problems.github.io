[#gui]
:imagesdir: chapters/gui/images
:programsdir: chapters/gui/programs
...
and making it function properly. <<gui-basics#gui-basics>>
...
As discussed in <<interfaces#interfaces>>, an interface is a set of
...
----
JButton thisButton = new JButton("This");
thisButton.addActionListener(e -> { /* Code to perform an action goes here */ });
----

Since almost everyone uses Java 8 or higher, we prefer the lambda expression
...
.GUI with actions using anonymous classes
====
...
.Demonstrates handling of action events with anonymous inner classes.
[source, java]
[[FrameWithPanelAndActionsClassesProgram]]
----
include::{programsdir}/FrameWithPanelAndActionsClasses.java[]
----

<<FrameWithPanelAndActionsClassesProgram>> is largely the same as
...
.GUI with actions using lambda expressions
====
...
.Demonstrates handling of action events with lambda expressions.
[source, java]
[[FrameWithPanelAndActionsLambdasProgram]]
----
include::{programsdir}/FrameWithPanelAndActionsLambdas.java[]
----
<.> Using only a single line of code, this lambda expression creates an
`ActionListener` that changes the text field to "`You can get with this.`"
<.> Similarly, this lambda expression creates an `ActionListener` that changes
the text field to "`Or you can get with that.`"
<.> The previous lambda expressions did not require braces, but this one does
since it contains multiple lines: one that prints "`Exit`" and one that disposes
the frame.
====
...
Just as `ActionListener` is a functional interface with a single method, 
...
<.> Using lambda expressions, we create an `ItemListener` for each check box
...
<.> Using lambda expressions, we create an `ItemListener` for each radio button.
...
<<mouseListenerExample>>, there's no need to implement all the methods because
...
----
<.> Class `SimpleMouseAdapter` extends the abstract class `MouseAdapter`. Thus,
it inherits an empty method for every method required by `MouseListener`.
<.> We override the method `mouseEntered()`.
<.> We override the method `mouseClicked()`.  No other methods need to be overridden in this example.
====

****
<<MouseAdapterExercise>>
****
...
https://download.oracle.com/javase/tutorial/uiswing/events[Swing Events tutorial site^].
...
checkBox.addItemListener(e -> { /* Code for check box */ });
----
...
[source, java]
----
include::{programsdir}/MathTutor.java[lines=19..42]
----
<.> Inside the `MathTutor` constructor, we create the frame and the remaining components.
<.> Action listeners are added to the four operations menu items. We could have used lambda expressions here, but it makes sense to share a single `actionPerformed()` method among all four operations since they all enable `submitButton` and clear `field`.
...
<.> Next, we add a lambda expression as an `ActionListener` just for the submit button. It
...
[source, java]
----
include::{programsdir}/MathTutor.java[lines=43..62]
----
...
[source, java]
----
include::{programsdir}/MathTutor.java[lines=64..76]
----
...
[source, java]
----
include::{programsdir}/MathTutor.java[lines=78..91]
----
...
[source, java]
----
include::{programsdir}/MathTutor.java[lines=93..96]
----
...
<<synchronization#synchronization>> will become important. However, only
...
<<dynamic-data-structures#dynamic-data-structures>> covers generics
...
https://download.oracle.com/javase/tutorial/uiswing/components[Swing Components tutorial site^] or other
reference sources.
...
. [[MouseAdapterExercise]] Why is the `MouseAdapter` abstract class useful?
...
`ActionListener`,`MouseListener`, and `ItemListener`.

*Programming Practice*

. Write a program that creates a GUI containing two buttons labeled "`Start`"
and "`Done.`" The GUI frame should be titled "`Start and Done.`"
...
visiting https://www.freesound.org[Freesound.org^] to find free sound files.
. [[selfieExercise]] Write a Java program that creates a GUI containing a label
with a picture of yourself.
. [[animalSoundsExercise]] Extend the `AnimalSounds` class developed in
<<animalSoundsExample>> to include sounds for various animals. There are many
publicly available sounds files for use in your program. The Freesound link
above is only one source.
...
individual lambda expressions added to the `add`, `subtract`,
...
<<FrameWithPanelAndActionsClassesProgram>> exchange the first two
