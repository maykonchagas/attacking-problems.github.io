...
We discussed constants in <<Constantes>>, but using the `final` keyword only
...
The C programming language introduced enumerations, fixed lists of
named constants starting at 0 and increasing by one for each value.
This approach provided an easy way to make distinct, named constants for better
readability. In C, these values were essentially integers, but a
conceptually similar feature was added in Java 5 to create Java enumerations,
fixed lists of named objects.
...
[source,java]
----
 Direction westSide = Direction.valueOf("WEST");
----

Doing so is risky, since the name must match exactly, including case.
Supplying an incorrect name will cause an error.

==== Customized enumeration types
...
----
<.> To use an `enum` type in this way, each value must be followed by
parentheses giving the values passed into the constructor.
<.> Then, the list of values must be terminated with a semicolon.
<.> The rest of the `enum` type looks like any other class, containing members,
...
After the code executes, the value of `x` will be `1`, and the value of `y`
will be `-1`.
...
Because both member variables are public, they can be accessed
directly, without needing a method. Mutable public member variables are generally
considered unwise, but public constants are both safe and useful.
...
This `enum` type could be generalized to include all the elements in the
...
[source, java]
----
include::{programsdir}/NestedExpressions.java[lines=10..28]
----
...
are discussed in <<exceptions#exceptions>>. Some critics of Java
...
<<concurrency#concurrency>>, we'll discuss how to create
...
In <<synchronization#synchronization>>, we'll discuss how to prevent this
...
and manipulate `int` values. In <<dynamic-data-structures#dynamic-data-structures>>,
