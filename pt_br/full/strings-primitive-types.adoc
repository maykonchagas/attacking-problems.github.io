...
which we will discuss in <<arrays#arrays>>. However, too narrow
...
precision discussed in <<computer-basics#computer-basics>>, Java cannot
...
As discussed in <<computer-basics#computer-basics>>, integer types
...
Note that the last visible digit is rounded instead of truncated. Note that `%n` is a special format specifier that indicates a newline. To learn about other ways to use format strings to manipulate output, read the Oracle https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/Formatter.html#syntax[Formatter documentation^].
...
should always strive to make your own code as readable as possible. Throughout
this book we will prefer the prefix style for increments and decrements,
because it's guaranteed to be as fast or faster than postfix and because it
matches what most people intuitively expect, that the update happens first. 
...
<<computer-basics#computer-basics>>. These operators perform bitwise
...
storage. From <<computer-basics#computer-basics>>, you know that you can
...
visit the https://www.unicode.org[Unicode site^] if you want more information.
...
<<selection#selection>>, <<repetition#repetition>>, and beyond.
...
----
<.> In the first use of `==`, the value of `same` is `true` because
the value of `x` is `3`.
<.> In the second, the value of `same` is `false` because the values of `x` and
`y` are different.
<.> The value of `xIsGreater` is also `false` since the value of `x` is not greater than
the value of `y`.

All the parentheses in this example are unnecessary and are used only for
clarity.
====
...
In <<intro#intro>>, you saw that we
...
it further in <<arrays#arrays>>. It may help if you think of
...
There are several other methods provided by `String` that we introduce as the need arises. If you are curious, you should look into the Java documentation for `String` in the Oracle https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/lang/String.html[String documentation^] for a complete list of available methods.
...
This section describes a change to the Java language introduced in Java 10.  Consider the following variable declarations that include an initial value assignment.
...
Use of `var` can result in clearer and more concise code, as long as the type being inferred by the compiler is obvious and what you intended. For example, if you intended `x` to be a `double`, you would need to use a declaration like one of the following.
...
Note that this feature is called _local variable type inference_.  The compiler
only does type inference on _local variables_, not on fields, method parameters,
or return types.
...
https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/lang/Math.html[Math documentation^]. Note that all angles are given in radians.
...
running a program. We discuss how to work with exceptions in <<exceptions#exceptions>>.

===== Numerical to `String` conversions

In some cases, the opposite conversion must be done: turning a number into an
equivalent `String`. A wrapper class can be used for this task as well.
Unsurprisingly, the method to convert a number to a `String` is the `toString()`
method, which can be called from a `Byte`, `Short`, `Integer`, `Long`, `Float`,
or `Double` wrapper class, depending on the type of the number.

[source, java]
----
String value = Integer.toString(68); // Contains "68"
----

Each integer wrapper class (`Byte`, `Short`, `Integer`, and `Long`) even has a
version of the `toString()` method that converts a number to a `String`
representation in a particular base. For example, the following method converts
`604` to its `String` representation in hexadecimal, `"25c"`.

[source, java]
----
String hex = Integer.toString(604, 16); // Contains "25c"
----

Except for this kind of base conversion, many programmers rarely use
`toString()` methods because there's an easier way to convert a number (or any
type) to a `String`. The concatenation operator (`+`) discussed in
<<Concatenating `String` values>> can be used to concatenate a number to the
empty string (`""`), “adding” a `String` version of the number to, well,
nothing. Using this style, you might see code like the following.

[source, java]
----
int number = 68;
String text = "" + number; // Contains "68"
----

While an argument can be made that using the `toString()` method is more
readable, the concatenation approach is certainly shorter to type and very
popular.
...
As you recall from <<computer-basics#computer-basics>>, integer
...
in <<dynamic-data-structures#dynamic-data-structures>>, where they are used to
...
<.> We start the second thread running. Every object whose type is `Thread` (or a child of `Thread`, which we will discuss in <<inheritance#inheritance>>) has a `start()` method which begins its execution as a separate thread.
...
<<exceptions#exceptions>>, but for now, you only need to know that
...
design, and inheritance that we'll cover in <<methods#methods>>, <<classes#classes>>, and <<inheritance#inheritance>>.
...
carefully, if it can be done at all. <<synchronization#synchronization>>
...
many websites such as https://www.asciitable.com[Ascii Table^].
