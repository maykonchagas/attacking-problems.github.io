...
variável `valor`. O símbolo `5` é um exemplo de um _literal_. Um literal
...
byte idade;
int numeroDeQuartos;
long contadorInfinito = 0;
----

A primeira dessas instruções declara `idade` como uma variável do tipo
...
Isso atribui o valor `32` à variável `idade`. Observe que o compilador Java
...
A primeira das duas instruções acima declara `areaDoQuarto` como sendo do tipo
...
O operador `pass:[+=]` obtém o conteúdo da variável, neste caso `valor`,
...
int posfixo = 7;
int resultadoPosfixo = 5 + postfix++;
----

Após a execução do código, os valores de `prefixo` e `posfixo` são
...
Para realizar um _downcast_, o programador precisa indicar que
pretende que a conversão ocorra. Um _downcast_ é indicado
...
int areaDaCasa = areaDoQuarto * 4.0;
----
...
----
double areaDoQuarto = 3.5;
int areaDaCasa = (int) (areaDoQuarto * 4.0);
----
...
[source, java]
----
int contagem = (int) Math.round(15.99999);
----
...
boolean igual = (x == 3); // <.>
igual = (x == y); // <.>
boolean xEhMaior = (x > y); // <.>
----
<.> No primeiro uso de `==`, o valor de `igual` é `true` porque
o valor de `x` é `3`.
<.> Na segunda, o valor de `igual` é `false` porque os valores de `x` e
`y` são diferentes.
<.> O valor de `xEhMaior` também é `false`, já que o valor de `x` não é maior que
o valor de `y`.
...
Alternativamente, um construtor `Vombate` pode permitir que você especifique sua massa em
...
[.text-center]
.Duas referências `Vombate` apontando para o mesmo objeto.
image::wombat.svg[scaledwidth=50%,pdfwidth=50%,width=50%]


Então, qualquer coisa que fazermos com `v1` afetará `v2` e vice-versa. Por
...
Talvez isso aumente a massa do objeto para o qual `v1` aponta para
...
|isUpperCase(char value)|Retorna `true` se `valor` for uma letra maiúscula
e `false` caso contrário.

|isWhitespace(char value)|Retorna `true` se `valor` for um caractere de espaço em branco
, como espaço, tabulação ou nova linha, e `false` caso contrário.

|toLowerCase(char value)|Retorna uma versão em minúsculas de `valor`,
sem alteração se não for uma letra.

|toUpperCase(char value)|Retorna uma versão em maiúscula de `valor`,
sem alteração se não for uma letra.
|=======================================================================
...
[source,java]
----
boolean teste = Character.isLetter("x");
----
