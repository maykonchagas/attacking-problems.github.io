...
:imagesdir: chapters/dynamic-data-structures/images
:programsdir: chapters/dynamic-data-structures/programs
== Dynamic Data Structures
...
<<classes#classes>> and examine in greater depth here.
...
problem from <<classes#classes>> was not actually a dynamic data
...
[source, java]
----
include::{programsdir}/SortedLinkedList.java[lines=40..53]
----
...
We've already used stacks to solve problems in <<classes#classes>>.
...
In this case, the type inferred for `list` is `GenericLinkedList<Object>`. Using
...
enhanced `for` loops described in <<Enhanced `for` loops>>. All classes that
...
<<ArrayListExampleProgram>> illustrates a use of the `ArrayList`
...
.Illustrates the use of the `ArrayList` class.
[source, java]
[[ArrayListExampleProgram]]
----
include::{programsdir}/ArrayListExample.java[]
----

Output from a typical run of <<ArrayListExampleProgram>> is shown
below.
...
we'll discuss in <<vectorExample>>, a `Vector` is similar to an `ArrayList` in
...
keys with values. You can think of it as _mapping_ a _key_ to a _value_. In the
...
To show a map in action, <<HashMapExampleProgram>> reads a sequence of lines
...
The following table summarizes several important JCF classes. All of these are
used frequently in professional Java code.

[.center%autowidth%unbreakable,cols="<.^,<.^,<.^,<.^",]
|=======================================================================
|Interface  |Use |Implementation |Details

.3+|`List` .3+|Stores sequential list of data
|`ArrayList` | Uses dynamic array
|`LinkedList` | Uses linked list
|`Vector` | Like `ArrayList` but thread-safe

.2+|`Map` .2+|Stores key-value pairs
|`HashMap`| Uses hash table
|`TreeMap`| Uses tree, allows ordering

.2+|`Set` .2+|Stores unique values
|`HashSet`| Uses hash table
|`TreeSet`| Uses tree, allows ordering
|=======================================================================
...
[source, java]
----
include::{programsdir}/Term.java[lines=20..33]
----
...
----
include::{programsdir}/UseLinkedListQueue.java[lines=1..31]
----
...
[source, java]
----
include::{programsdir}/UseLinkedListQueue.java[lines=33..49]
----
...
<<synchronization#synchronization>>, this mutual exclusion can be
...
thread-safe and some are not. The JCF is a useful library, but it's also a
library that requires you to manage your own thread safety if it matters for
your program.
...
As we mentioned in <<interfaces#interfaces>>, an interface can't
...
A `Vector` is like an `ArrayList`, with essentially the same interface
...
<<VectorExampleProgram>> is an example that makes updates to a `Vector` class
with multiple threads.

.Example of thread-safe use of an `Vector`.
[source, java]
[[VectorExampleProgram]]
----
include::{programsdir}/VectorExample.java[lines=1..20]
----
<.> The `main()` method creates a `Vector`.
<.> It then creates and starts two threads, passing in the `Vector` as an
...
[source, java]
----
include::{programsdir}/VectorExample.java[lines=22..36]
----
<.> The constructor stores a reference to the shared `Vector`.
...
<<ReadAndGrowArrayProgram>>, is it possible to increment
